...
#[async_trait]
impl KairoPlugin for MemoryStorePlugin {
    fn meta(&self) -> PluginMeta {
        PluginMeta {
            id: "memory-store".into(),
            name: "Memory Store Plugin".into(),
            version: "0.1.0".into(),
            dependencies: vec![],
    ...
    async fn load(&mut self, config: toml::Value) -> Result<()> {
        tracing::info!("MemoryStorePlugin loaded");
        self.config = serde_json::to_value(config.to_string())?;
        Ok(())
    }

    async fn init(&mut self) -> Result<()> {
        tracing::info!("MemoryStorePlugin initialized");
        Ok(())
    }

    async fn start(&mut self) -> Result<()> {
        tracing::info!("MemoryStorePlugin started");
        Ok(())
    }

    async fn stop(&mut self) -> Result<()> {
        tracing::info!("MemoryStorePlugin stopped");
        Ok(())
    }
